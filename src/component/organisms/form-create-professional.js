...
import { getProfessionalTypeData } from "../../services/professionalTypeService";
import { saveProfessionalData } from "../../services/professionalService";

let ModalFormCreateProfessional = ({ userId }) => {
    ...
    const {
        data: professionalTypeData,
        isSuccess: getProfessionalTypeDataSuccess
    } = useQuery({
        queryKey: ["professional-type"],
        queryFn: getProfessionalTypeData,
        enabled: true,
        staleTime: 300000,
        cacheTime: 300000,
    ...
    useEffect(() => {
        if (professionalTypeData?.data !== undefined && getProfessionalTypeDataSuccess) {
            setType(professionalTypeData.data)
        }
    }, [professionalTypeData, getProfessionalTypeDataSuccess])

    // useEffect(() => {
    //     console.log(professionalTypeData?.data);
    ...
    const handleChange = (e) => {
        ...
        setRequest(prev => ({
            ...prev,
            [name]: name === "profesionalTypeId" ? parseInt(value) : value
        }))
    }
    ...
    const {
        mutate: addProfessionalData,
        isLoading
    } = useMutation({
        mutationFn: saveProfessionalData,
        onSuccess: () => {
        ...
        <form action="#" className="form-horizontal form-bordered">
            <div className="form-body">
                ...
                <div className="form-group row">
                    ...
                    <div className="col-md-9">
                        <select className="form-control custom-select" value={request.profesionalTypeId || ""} name="profesionalTypeId" onChange={handleChange} >
...
export default ModalFormCreateProfessional;